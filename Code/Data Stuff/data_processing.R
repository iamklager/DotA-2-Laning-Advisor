...
## Attack
dt_HeroStats[, AS := floor((AS_b + AGI)*(1 + ASM))]
dt_HeroStats[, AS := ifelse(AS < 20, 20, AS)]
dt_HeroStats[, AS := ifelse(AS > 700, 700, AS)]
dt_HeroStats[, AtkRt := round(AS/(100*AtkT_b), 3)]
...
dt_HeroStats[, DMG_min := floor(DMG_min_b + DMG_main)]
dt_HeroStats[, DMG_max := floor(DMG_max_b + DMG_main)]
...
dt_HeroStats[, MagRes := round(MagRes_b + (INT/10)*0.01, 3) ]
dt_HeroStats[, SlwRes := round(SlwRes_b, 3) ]
dt_HeroStats[, StsRes := round(StsRes_b, 3) ]
dt_HeroStats[, ARM := round(ARM_b + (AGI*0.167), 1) ]
